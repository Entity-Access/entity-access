...
export default async function(this: TestConfig) {
    ...
    //     .where({} , (x, p) => x.profile.photos.some((a) => true) || x.profile.photos.some((a) => true))
    //     .count();
    ...
    let all = await context.users.all()
        .where({} , (x, p) => x.profile.photos.some((a) => a.photoID > 0) || x.profile.photos.some((a) => a.photoID > 0))
    ...
    all = await context.users.all()
        .where({} , (x, p) => x.files.some((a) => a.fileID > 0))
        .include((x) => x.files)
        .first();
